...
PHIHttp::PHIHttp( PHIObject *parent )
    : PHIObject( parent ), _state( PHIHttp::Connected ), _req( 0 )
{
    //qDebug( "PHIHttp::PHIHttp()" );
    _timer=new QTimer( this );
    connect( _timer, SIGNAL( timeout() ), this, SIGNAL( close() ) );
    _timer->setInterval( config()->keepAlive() );
    _timer->start();
}
...
PHIRC PHIHttp::init( qintptr socketDesc, bool usessl, QString &err )
{
    ...
    try {
        _req=new PHIServerRequest( &_server, &_resp, this );
    } catch ( std::bad_alloc& ) {
        emit log( PHILOGCRIT, PHIRC_OUT_OF_MEMORY, err=tr( "Could not init request. Out of memory." ) );
        return PHIRC_OUT_OF_MEMORY;
    }
    //qDebug( "PHIHttp::init() - end" );
    return PHIRC_OK;
}

void PHIHttp::slotSslErrors( const QList<QSslError> &errors )
{
...
void PHIHttp::sendError( PHIRC rc, const QString &reason )
{
...
PHIRC PHIHttp::sendResponse( QString &err )
{
#ifdef PHIDEBUG
    ...
    if ( _req->canonicalFilename().endsWith( QLatin1String( "/phi.phis" ) ) ) {
        if ( _req->hostname()==QLatin1String( "localhost" ) ) { // only accepted from localhost
            ...
            if ( serviceCommand ) {
                qDebug( "DocumentRoot=%s", qPrintable( _req->documentRoot() ) );
                _socket->write( _resp.createResponseHeader( PHIRC_HTTP_FORBIDDEN, _server._name ) );
                _socket->write( "\r\n" );
                _socket->flush();
                return PHIRC_OK;
            }
...
PHIRC PHIHttp::readUrlEncoded( QString &err )
{
    Q_UNUSED( err );
...
PHIRC PHIHttp::readMultipartEncoded( QString &err )
{
...
PHIRC PHIHttp::readPhiEncoded( QString &err )
{
...
PHIRC PHIHttp::parseRequestHeader( const QString &headerStr, QString &err )
{
